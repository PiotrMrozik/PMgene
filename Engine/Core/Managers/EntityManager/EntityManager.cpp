...
namespace PMgene::Core
{
	EntityManager::EntityManager(
		...
		ISender(communicationManagerInput, "EntityManager"), ISubscriber()
	{
		...
		SendMessage(std::make_shared<Message>(INFO, MC_MANAGER_INITIALIZED, senderName));
	}
	...
	Entity EntityManager::CreateEntity()
	{
		...
		{
			SendMessage(std::make_shared<Message>(ERROR, MC_ENTITY_MAX_LIMIT_REACHED, senderName));
			return -1;
		}
		...
		if (!availableEntities.empty())
		{
			...
			SendMessage(std::make_shared<Message>(INFO, MC_ENTITY_CREATED, "Entity Manager"));
			return entity;
		}
		return -1;
	}
